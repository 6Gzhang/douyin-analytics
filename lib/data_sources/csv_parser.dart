  ...
  static List<FeishuDouyinMetric> parseDouyinData(
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      results.add(FeishuDouyinMetric(
        videoTitle: _get(map, ['视频标题', '标题', 'video_title', 'title']),
        playCount: _getInt(map, ['播放量', '播放数', 'play_count', 'plays']),
        likeCount: _getInt(map, ['点赞数', '点赞', 'like_count', 'likes']),
        commentCount: _getInt(map, ['评论数', '评论', 'comment_count', 'comments']),
        shareCount: _getInt(map, ['分享数', '分享', 'share_count', 'shares']),
        collectCount: _getInt(map, ['收藏数', '收藏', 'collect_count', 'collects']),
        publishDate: _get(map, ['发布时间', '发布日期', 'publish_date', 'date']),
        finishRate: _getDouble(map, ['完播率', 'finish_rate']),
        avgWatchDuration: _getDouble(map, ['平均观看时长', 'avg_watch_duration']),
      ));
    }
    return results;
  }
